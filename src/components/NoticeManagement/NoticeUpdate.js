...
const NoticeUpdate = () => {
  ...
  useEffect(() => {
    const fetchNotice = async () => {
      ...
      const response = await axios.get(`https://luckytang-server.onrender.com/api/notice/${id}`, {
      ...
      if (response.data.success) {
        ...
        if (response.data.notice.noticeImage?.length > 0) {
          setNoticeImagePreview(`https://luckytang-server.onrender.com${response.data.notice.noticeImage[0]}`);
        }
      }
    };
    fetchNotice();
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await axios.put(`https://luckytang-server.onrender.com/api/notice/${id}`, formData, {
        headers: {
