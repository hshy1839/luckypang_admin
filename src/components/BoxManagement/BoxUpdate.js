...
const BoxUpdate = () => {
  ...
  useEffect(() => {
    const fetchBox = async () => {
      ...
      const response = await axios.get(`http://localhost:7778/api/box/${id}`, {
      ...
      if (response.data.success) {
        ...
        if (box.mainImage) setMainImagePreview(`http://localhost:7778${box.mainImage}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`http://localhost:7778/api/box/${id}`, formData, {
        headers: {
          ...
          <div className="image-preview-list">
            {initialAdditionalImages.map((url, i) => (
              <div key={`initial-${i}`} className="preview-item">
                <img src={`http://localhost:7778${url}`} alt="기존 상세 이미지" />
