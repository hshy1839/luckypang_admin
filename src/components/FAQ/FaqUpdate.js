...
const FaqUpdate = () => {
  ...
  useEffect(() => {
    const fetchFaq = async () => {
      ...
      const response = await axios.get(`https://luckytang-server.onrender.com/api/faq/detail/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await axios.put(`https://luckytang-server.onrender.com/api/faq/${id}`, form, {
        headers: {
