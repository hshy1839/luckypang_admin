...
const RefundTermEdit = () => {
  ...
  useEffect(() => {
    const fetchTerm = async () => {
      try {
        const response = await axios.get('https://luckytang-server.onrender.com/api/terms/refundTerm');
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        'https://luckytang-server.onrender.com/api/terms/refundTerm', // ✅ 이 경로가 백엔드 라우터와 일치
        { content },
  ...
  return (
    <div className="product-detail-container">
      <h1 className="product-name">교환, 환불 정책</h1>
